...
export const issuesReducer: Reducer<IssuesReducerType> = (
  ...
  switch (action.type) {
    ...
    case `${API_ACTIONS.FETCH_START}${GET_ALL_ISSUES_FILTERED_ASYNC}`:
      return { ...state, loading: true, error: undefined };

    case `${API_ACTIONS.FETCH_FAILURE}${FETCH_ISSUE_BY_ID}`:
    case `${API_ACTIONS.FETCH_FAILURE}${GET_ALL_ISSUES_ASYNC}`:
    case `${API_ACTIONS.FETCH_FAILURE}${GET_ALL_ISSUES_FILTERED_ASYNC}`:
    ...
    case `${API_ACTIONS.FETCH_SUCCESS}${GET_ALL_ISSUES_FILTERED_ASYNC}`:
